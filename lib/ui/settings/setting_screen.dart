...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _load() async {
    ...
    _appCtl.text = apiClient.appName ?? 'proxy';
    _portCtl.text = (apiClient.port != null)
        ? apiClient.port.toString()
        : '8780';
    _phoneCtl.text = apiClient.phone ?? '';
    _addrCtl.text = apiClient.address ?? '';
    setState(() => isRemote = apiClient.isRemote);
  }
  ...
  Future<void> _save() async {
    ...
    apiClient.phone = _phoneCtl.text.trim();
    apiClient.address = _addrCtl.text.trim();

    if (!mounted) return;

    showSnack('Configuration enregistrée');

    // Si l'utilisateur est connecté, logout pour recharger token si nécessaire
